## Module AWS.CloudSearchDomain.Requests

#### `search`

``` purescript
search :: forall eff. Service -> SearchRequest -> Aff (exception :: EXCEPTION | eff) SearchResponse
```
...
``` purescript
suggest :: forall eff. Service -> SuggestRequest -> Aff (exception :: EXCEPTION | eff) SuggestResponse
```
...
``` purescript
uploadDocuments :: forall eff. Service -> UploadDocumentsRequest -> Aff (exception :: EXCEPTION | eff) UploadDocumentsResponse
```
